...
一个基于 Live2D 模型的透明桌面宠物：实时渲染、眼神跟随鼠标，并且能感知 Codex 的运行状态自动切换动作和表情；还内置了角色聊天模式，可以像朋友一样和角色对话。内置 **yumi** 和 **悠小喵** 两个模型，支持随时切换。
...
- **Codex 状态联动**：读取 Codex 实时日志，自动切换状态
  - 待机 → 爱心眼 + 待机摇摆（yumi）/ 常规动作（悠小喵）
  - 思考中 → 星星眼 + 托腮思考（yumi）/ 星星眼 + 扶脸（悠小喵）
  - 执行命令 → 蚊香眼 + 俯身忙碌（yumi）/ 晕晕眼 + 记笔记（悠小喵）
  - 完成 → 一次挥手
  - 故障 → 黑脸
  - 状态切换带柔和过渡动画，头顶气泡提示（思考中…/执行命令…/完成！/故障！）
- **模型切换**：右键 →「模型」子菜单可随时切换 yumi / 悠小喵；当前模型会自动保存
- **动作预设 + 手动选择**：模型自带动作时自动采用（如悠小喵的常规动作、扶脸、记笔记等），没有预设的状态回退到内置动作引擎
...
├── model/
│   ├── yumi/                # yumi 模型 + model.json 配置
│   └── youxiaomiao/         # 悠小喵模型 + model.json 配置
├── tools/web/
...
- 更省事的方式：右键 →「模型」→「导入模型（zip）」直接选择 Live2D 模型压缩包（如悠小喵那种），或「导入模型（文件夹）」选择已解压的模型目录；程序会自动识别表情（星星眼/黑脸/脸红等）、姿势（扶脸/记笔记/看手机等）和动作，生成配置并切换到新模型
